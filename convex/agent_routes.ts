...
export function registerAgentRoutes(http: HttpRouter) {
  ...
  http.route({ path: "/api/agent/business", method: "PATCH", handler: agentSecured(async (ctx, request, businessId) => { const input = await body(request); if (typeof input.name !== "string" || input.name.length > 120 || !input.name.trim()) throw new ConvexError({ code: "VALIDATION_ERROR", message: "Nama ruang kerja wajib diisi." }); await ctx.runMutation(internal.agent.updateBusinessFromToken, { businessId, name: input.name }); return json({ ok: true }); }) });
