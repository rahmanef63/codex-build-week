...
// Mode Real: setiap user memiliki satu ruang kerja; businessId = userId sehingga
...
const logChange = (ctx: MutationCtx, businessId: string, action: string, outputSummary: string) =>
  ctx.db.insert("aiActionLogs", { businessId, action, inputSummary: "Perubahan dari dashboard ruang kerja.", outputSummary, requiresVerification: true, createdAt: Date.now() });
...
export const createBusiness = mutation({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      if (!name) {
        fail("INVALID_NAME", "Nama ruang kerja wajib diisi.");
      }
      const existing = await ctx.db
        ...
        .unique();
      if (existing) {
        fail("BUSINESS_EXISTS", "Ruang kerja Anda sudah terdaftar.");
      }
      ...
      await logChange(ctx, userId, "create_business", "Ruang kerja baru dibuat.");
      return { ok: true };
    } catch (error) {
...
export const updateBusiness = mutation({
  ...
  handler: async (ctx, args) => {
    const userId = await requireUserId(ctx);
    const name = args.name.trim();
    if (!name) fail("INVALID_NAME", "Nama ruang kerja wajib diisi.");
    const business = await ctx.db.query("businesses").withIndex("by_business_id", (q) => q.eq("businessId", userId)).unique();
    if (!business) fail("BUSINESS_NOT_FOUND", "Buat ruang kerja terlebih dahulu.");
    await ctx.db.patch(business._id, { name });
    await logChange(ctx, userId, "update_business", "Profil ruang kerja diperbarui.");
  },
});
