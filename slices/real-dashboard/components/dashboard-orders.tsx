      ...
      <section className="border-y border-border py-4 sm:py-5">
        ...
        {open ? (
          ...
          >
            ...
            <Button className="min-h-11" disabled={busy} type="submit">{busy ? "Menyimpan…" : "Simpan pesanan"}</Button>
          </form>
        ) : null}
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full text-sm">
                ...
                <thead className="border-b border-border text-left text-xs text-muted-foreground">
                  <tr>{["Nama kontak", "Item", "Ambil", "Pembayaran", "Status", "Total"].map((label) => <th className="px-6 py-3 font-medium last:text-right" key={label}>{label}</th>)}</tr>
                </thead>
                <tbody>
                  {orders.map((order) => (
