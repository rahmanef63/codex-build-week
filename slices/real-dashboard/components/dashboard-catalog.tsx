...
export function DashboardCatalog({ products }: { products: DashboardData["products"] }) {
  ...
  return <div className="max-w-6xl space-y-5">
    <section className="rounded-md border border-border bg-card p-5"><h2 className="text-sm font-semibold">Tambah produk</h2><form className="mt-4 grid gap-3 sm:grid-cols-5" onSubmit={async (event) => { event.preventDefault(); setError(""); try { await create(values(draft)); setDraft(blank()); } catch { setError(failure); } }}>
    ...
    <section className="overflow-hidden rounded-md border border-border bg-card"><div className="border-b border-border px-5 py-4"><h2 className="text-sm font-semibold">Katalog produk</h2><p className="text-xs text-muted-foreground">Tambah, ubah, atau hapus produk usaha Anda.</p></div><div className="divide-y divide-border">{products.map((product) => { const value = editing[product._id] ?? { name: product.name, price: String(product.price), stock: String(product.stock), lowStockThreshold: String(product.lowStockThreshold) }; const edit = (patch: Partial<Draft>) => setEditing({ ...editing, [product._id]: { ...value, ...patch } }); const save = async () => { setError(""); try { await update({ productId: product._id as any, ...values(value) }); setEditing(({ [product._id]: _, ...rest }) => rest); } catch { setError(failure); } }; const destroy = async () => { if (!window.confirm(`Hapus ${product.name}?`)) return; setError(""); try { await remove({ productId: product._id as any }); } catch { setError("Produk tidak dapat dihapus."); } }; return <div className="grid gap-2 p-4 sm:grid-cols-[2fr_1fr_1fr_1fr_auto]" key={product._id}>{(["name", "price", "stock", "lowStockThreshold"] as const).map((key) => <input key={key} className="rounded-md border border-input bg-background px-3 py-2 text-sm" value={value[key]} onChange={(event) => edit({ [key]: event.target.value })} type={key === "name" ? "text" : "number"} min={0} aria-label={`${key} ${product.name}`} />)}<div className="flex gap-2"><Button size="sm" onClick={() => void save()}>Simpan</Button><Button size="sm" variant="destructive" onClick={() => void destroy()}>Hapus</Button></div></div>; })}{!products.length ? <p className="p-5 text-sm text-muted-foreground">Belum ada produk.</p> : null}</div></section>
  </div>;
}
