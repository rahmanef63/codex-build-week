    ...
    <div className="w-full space-y-5">
      <section className="border-y border-border py-4 sm:py-5">
        ...
        >
          ...
          <Button className="min-h-11" type="submit">Tambah</Button>
        </form>
                ...
                {isEditing ? (
                  <div className="grid gap-2 sm:grid-cols-[2fr_1fr_1fr_1fr_auto]">
                    ...
                    <div className="flex items-end gap-2">
                      <Button className="min-h-11" onClick={() => void save(product._id)} size="sm">Simpan</Button>
                      <Button className="min-h-11" onClick={() => setEditing(null)} size="sm" variant="outline">Batal</Button>
                    </div>
                  </div>
                ) : (
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs text-muted-foreground break-words">{formatRupiah(product.price)} · stok {product.stock} · batas minimum {product.lowStockThreshold}</p>
                    </div>
                    <div className="flex shrink-0 gap-2">
                      <Button className="pointer-coarse:min-h-11" onClick={() => { setEditing(product._id); setEditDraft(fromProduct(product)); }} size="sm" variant="outline"><Pencil />Edit</Button>
                      <Button className="pointer-coarse:min-h-11" onClick={() => setDeleting(product)} size="sm" variant="ghost"><Trash2 />Hapus</Button>
                    </div>
                  </div>
                )}
              </article>
      ...
      {deleting ? (
        ...
        <div className="fixed inset-0 z-60 grid place-items-center bg-black/60 p-5" role="presentation" onMouseDown={(event) => { if (event.currentTarget === event.target) setDeleting(null); }}>
          <section aria-describedby="delete-product-description" aria-labelledby="delete-product-title" aria-modal="true" className="w-full max-w-sm rounded-lg border border-border bg-card p-6 shadow-xl" role="alertdialog">
            <h2 className="text-lg font-semibold break-words" id="delete-product-title">Hapus {deleting.name}?</h2>
            ...
            <div className="mt-6 flex flex-wrap justify-end gap-2">
              <Button autoFocus className="min-h-11" onClick={() => setDeleting(null)} variant="outline">Batal</Button>
              <Button className="min-h-11" variant="destructive" onClick={async () => { try { await remove({ productId: deleting._id }); setDeleting(null); } catch { setError("Produk tidak dapat dihapus."); } }}>Hapus produk</Button>
            </div>
          </section>
        </div>
      ) : null}
