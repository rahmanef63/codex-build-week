...
export function AgentSetup() {
  ...
  return <section className="w-full border-y border-border py-6">
    ...
    <div className="mt-6 flex max-w-full overflow-x-auto border-b border-border" role="tablist" aria-label="Detail GPT Builder">{fields.map(([id, label]) => <button className={`shrink-0 border-b-2 px-3 py-2 text-sm ${tab === id ? "border-accent text-foreground" : "border-transparent text-muted-foreground hover:text-foreground"}`} key={id} onClick={() => setTab(id)} role="tab" aria-selected={tab === id}>{label}</button>)}</div>
