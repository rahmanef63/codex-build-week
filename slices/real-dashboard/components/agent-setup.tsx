...
export function AgentSetup() {
  ...
  return <section className="max-w-3xl rounded-md border border-border bg-card p-6">
    ...
    <div className="rounded-md border border-border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Action schema dan token</h3><p className="mt-1 text-xs text-muted-foreground">Tempel schema ke Action. Token hanya untuk field API Key dengan custom header.</p>{config.token ? <p className="mt-2 text-xs text-muted-foreground">Token aktif: {config.token.prefix}… · dibuat {new Date(config.token.createdAt).toLocaleDateString("id-ID")} · {config.token.expiresAt ? `berakhir ${new Date(config.token.expiresAt).toLocaleDateString("id-ID")}` : "tanpa masa berlaku"}</p> : null}</div><div className="flex gap-2"><Button size="sm" onClick={async () => { const result = await issue(); setToken(result.token); setTokenExpiresAt(result.expiresAt); }}>Buat / rotasi token</Button><Button size="sm" variant="outline" onClick={() => void copy("schema", schema)}>{copied === "schema" ? <Check /> : <Copy />}{copied === "schema" ? "Tersalin" : "Salin schema"}</Button></div></div>{token && <div className="mt-4 rounded-md border border-accent bg-secondary p-4"><p className="text-sm font-semibold">Token satu-kali / One-time token</p><p className="mt-1 text-xs text-muted-foreground">Simpan sekarang; token berakhir {tokenExpiresAt ? new Date(tokenExpiresAt).toLocaleDateString("id-ID") : "sesuai kebijakan"}.</p><code className="mt-2 block break-all text-xs">{token}</code><Button className="mt-3" onClick={() => void copy("token", token)} size="sm">{copied === "token" ? <Check /> : <Copy />}{copied === "token" ? "Tersalin" : "Salin token"}</Button></div>}<pre className="mt-5 max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-md bg-muted p-4 text-xs">{schema}</pre></div></div>
  </section>;
}
