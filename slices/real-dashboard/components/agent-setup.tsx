...
function actionSchema(businessName: string, serverUrl: string) {
  ...
  return {
    ...
    paths: {
      ...
      "/api/agent/business": { get: { operationId: "get_business_profile", summary: "Baca profil usaha", description: "Membaca profil usaha pemilik token.", "x-openai-isConsequential": false, responses: { "200": { description: "Profil usaha." }, "401": error } }, patch: { operationId: "update_business_profile", summary: "Ubah nama usaha", description: "Setelah konfirmasi pengguna, memperbarui nama usaha pemilik token.", "x-openai-isConsequential": true, requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["name"], properties: { name: { type: "string" } } } } } }, responses: { "200": { description: "Profil diperbarui." }, "400": error, "401": error } } },
      "/api/agent/products": { get: { operationId: "list_products", summary: "Daftar katalog produk", description: "Membaca seluruh katalog produk usaha pemilik token.", "x-openai-isConsequential": false, responses: { "200": { description: "Katalog produk." }, "401": error } }, post: { operationId: "create_product", summary: "Tambah produk", description: "Setelah konfirmasi pengguna, menambah produk ke katalog usaha pemilik token.", "x-openai-isConsequential": true, requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["name", "price", "stock", "lowStockThreshold"], properties: { name: { type: "string" }, price: { type: "number", minimum: 0 }, stock: { type: "number", minimum: 0 }, lowStockThreshold: { type: "number", minimum: 0 } } } } } }, responses: { "201": { description: "Produk ditambahkan." }, "400": error, "401": error } } },
      "/api/agent/products/{id}": { patch: { operationId: "update_product", summary: "Ubah produk", description: "Setelah konfirmasi pengguna, memperbarui produk milik usaha pemilik token.", "x-openai-isConsequential": true, parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["name", "price", "stock", "lowStockThreshold"], properties: { name: { type: "string" }, price: { type: "number", minimum: 0 }, stock: { type: "number", minimum: 0 }, lowStockThreshold: { type: "number", minimum: 0 } } } } } }, responses: { "200": { description: "Produk diperbarui." }, "400": error, "401": error, "404": error } }, delete: { operationId: "delete_product", summary: "Hapus produk", description: "Setelah konfirmasi pengguna, menghapus produk milik usaha pemilik token.", "x-openai-isConsequential": true, parameters: [{ name: "id", in: "path", required: true, schema: { type: "string" } }], responses: { "200": { description: "Produk dihapus." }, "401": error, "404": error } } },
    },
...
export function AgentSetup() {
  ...
  return <section className="max-w-3xl rounded-md border border-border bg-card p-6">
    <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-medium uppercase tracking-widest text-accent">Integrasi GPT</p><h2 className="mt-2 text-xl font-semibold">Hubungkan GPT ke {config.businessName}</h2><p className="mt-2 text-sm text-muted-foreground">Salin detail berikut ke halaman konfigurasi GPT Builder. Isinya sudah disesuaikan dengan usaha Anda.</p></div><KeyRound className="text-accent" /></div>
    ...
    <div className="mt-6 space-y-4 md:hidden">{fields.filter(([id]) => id !== "integration").map(([id, label, value]) => <div key={id} className="rounded-md border border-border bg-muted/30 p-4"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-semibold">{label}</h3><Button size="sm" variant="outline" onClick={() => void copy(id, value)}>{copied === id ? <Check /> : <Copy />}{copied === id ? "Tersalin" : "Salin"}</Button></div><pre className="mt-3 max-h-52 overflow-auto whitespace-pre-wrap break-words text-xs text-muted-foreground">{value}</pre></div>)}</div>
    {tab !== "integration" ? <div className="mt-6 hidden rounded-md border border-border bg-muted/30 p-5 md:block" role="tabpanel"><div className="flex items-center justify-between gap-3"><h3 className="text-sm font-semibold">{active[1]}</h3><Button size="sm" variant="outline" onClick={() => void copy(active[0], active[2])}>{copied === active[0] ? <Check /> : <Copy />}{copied === active[0] ? "Tersalin" : "Salin semua"}</Button></div>{active[0] === "starters" ? <div className="mt-4 space-y-2">{details.starters.map((starter, index) => <div className="flex items-center justify-between gap-3 rounded-md border border-border bg-background p-3 text-sm" key={starter}><span>{starter}</span><Button size="xs" variant="outline" onClick={() => void copy(`starter-${index}`, starter)}>{copied === `starter-${index}` ? <Check /> : <Copy />}{copied === `starter-${index}` ? "Tersalin" : "Salin"}</Button></div>)}</div> : <pre className="mt-4 max-h-80 overflow-auto whitespace-pre-wrap break-words text-xs text-muted-foreground">{active[2]}</pre>}</div> : null}
    <div className={`mt-6 space-y-6 ${tab === "integration" ? "md:block" : "md:hidden"}`}><div className="rounded-md border border-border p-4"><div className="flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Pengetahuan GPT</h3><p className="mt-1 text-xs text-muted-foreground">Opsional: unduh lalu unggah ke bagian Knowledge di GPT Builder.</p></div><Button size="sm" variant="outline" onClick={downloadKnowledge}><Download />Unduh .md</Button></div></div>
    <div className="rounded-md border border-border p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-sm font-semibold">Schema Action dan token</h3><p className="mt-1 text-xs text-muted-foreground">Tempel schema pada bagian Action. Masukkan token hanya sebagai API Key dengan custom header.</p>{config.token ? <p className="mt-2 text-xs text-muted-foreground">Token aktif: {config.token.prefix}… · dibuat {new Date(config.token.createdAt).toLocaleDateString("id-ID")} · {config.token.expiresAt ? `berakhir ${new Date(config.token.expiresAt).toLocaleDateString("id-ID")}` : "tanpa masa berlaku"}</p> : null}</div><div className="flex gap-2"><Button size="sm" onClick={async () => { const result = await issue(); setToken(result.token); setTokenExpiresAt(result.expiresAt); }}>Buat token baru</Button><Button size="sm" variant="outline" onClick={() => void copy("schema", schema)}>{copied === "schema" ? <Check /> : <Copy />}{copied === "schema" ? "Tersalin" : "Salin schema"}</Button></div></div>{token && <div className="mt-4 rounded-md border border-accent bg-secondary p-4"><p className="text-sm font-semibold">Token hanya ditampilkan sekali</p><p className="mt-1 text-xs text-muted-foreground">Salin sekarang. Token berakhir {tokenExpiresAt ? new Date(tokenExpiresAt).toLocaleDateString("id-ID") : "sesuai kebijakan"}.</p><code className="mt-2 block break-all text-xs">{token}</code><Button className="mt-3" onClick={() => void copy("token", token)} size="sm">{copied === "token" ? <Check /> : <Copy />}{copied === "token" ? "Tersalin" : "Salin token"}</Button></div>}<pre className="mt-5 max-h-72 overflow-auto whitespace-pre-wrap break-words rounded-md bg-muted p-4 text-xs">{schema}</pre></div></div>
  </section>;
}
