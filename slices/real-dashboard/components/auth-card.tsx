...
export function AuthCard() {
  ...
  const titles: Record<Flow, string> = {
    signUp: "Mulai kelola usaha Anda",
  ...
  return (
    <div className="mx-auto grid min-h-[calc(100vh-80px)] w-full max-w-4xl items-start gap-8 py-4 md:grid-cols-[1fr_420px] md:items-center md:py-8">
      <section className="hidden space-y-6 md:block">
        <span className="dash-eyebrow">Dashboard usaha pribadi</span>
        <h1 className="max-w-lg text-4xl font-semibold tracking-tight">Operasional rapi, bahkan sebelum GPT dihubungkan.</h1>
        <p className="max-w-lg text-sm leading-relaxed text-muted-foreground">Catat pesanan manual, pantau stok, lalu hubungkan GPT saat Anda siap. Data setiap usaha tetap terisolasi.</p>
      ...
      <section aria-labelledby="dash-auth-title" className="dash-card dash-auth">
        ...
        <p className="dash-muted">{needsCode ? `Kami mengirim kode 6 digit ke ${email}.` : flow === "reset" ? "Masukkan email akun untuk menerima kode pemulihan." : "Pesanan, stok, dan aktivitas usaha tersimpan dalam satu dashboard."}</p>
        <form
          className="dash-form"
        ...
        >
          {!needsCode ? <label className="dash-label">Email<input autoComplete="email" className="dash-input" name="email" onChange={(event) => setEmail(event.target.value)} placeholder="nama@usaha.id" required type="email" value={email} /></label> : <label className="dash-label">Kode verifikasi<input autoComplete="one-time-code" className="dash-input" inputMode="numeric" maxLength={6} name="code" pattern="[0-9]{6}" required /></label>}
        ...
        <div className="flex w-full flex-wrap items-center justify-between gap-3">
          <Button className="dash-link h-auto" onClick={() => setFlow(flow === "signUp" ? "signIn" : "signUp")} type="button" variant="link">{flow === "signUp" ? "Sudah punya akun? Masuk" : "Kembali ke masuk"}</Button>
          {flow === "signIn" && capabilities?.emailVerification ? <Button className="dash-link h-auto" onClick={() => setFlow("reset")} type="button" variant="link">Lupa kata sandi?</Button> : null}
        </div>
