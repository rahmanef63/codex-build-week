            ...
            <li className="flex gap-4 px-5 py-4 sm:px-6" key={item._id}>
              ...
              <div className="min-w-0 flex-1 space-y-2">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex min-w-0 flex-wrap items-center gap-2"><h3 className="text-sm font-medium break-words">{formatAction(item.action)}</h3><span className="text-[11px] text-muted-foreground">{isRead ? "Membaca data" : "Mengubah data"}</span></div>
                  <time className="text-xs text-muted-foreground" dateTime={new Date(item.createdAt).toISOString()}>
                    {formatDateTime(item.createdAt)}
                  </time>
                </div>
                <p className="text-sm text-muted-foreground break-words">{formatActivitySummary(item.outputSummary)}</p>
