  ...
  return (
    <section className="max-w-6xl rounded-md border border-border bg-card">
      <div className="flex items-center justify-between gap-4 border-b border-border px-6 py-4">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2"><input aria-label="Cari operasi" className="w-28 rounded-md border border-input bg-background px-2 py-1 text-xs" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari operasi" /><select aria-label="Filter aktivitas" className="rounded-md border border-input bg-background px-2 py-1 text-xs" value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="all">Semua</option><option value="read">Baca GPT</option><option value="write">Perubahan</option></select><select aria-label="Rentang waktu" className="rounded-md border border-input bg-background px-2 py-1 text-xs" value={period} onChange={(event) => setPeriod(event.target.value as typeof period)}><option value="all">Semua waktu</option><option value="day">24 jam</option></select><span className="text-xs text-muted-foreground">{visible.length} aktivitas</span></div>
      </div>
      {visible.length ? (
