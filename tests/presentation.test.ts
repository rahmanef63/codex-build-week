...
// A ChatGPT GPT URL is "<id>-<slug>". Only the id resolves the GPT; the slug is
// cosmetic and chatgpt.com redirects a stale one. The deck link carries the
// current slug, while chatgpt-gpt-qr.svg still encodes the pre-rename slug — so
// the invariant that matters is that both point at the SAME GPT id.
const gptId = "g-6a5b0a5ef31c819181f8a68b5536d33e";
const gptUrl = `https://chatgpt.com/g/${gptId}-asisten-waringa`;
const slides = [
  "index.html",
  "01-masalah.html",
  "02-solusi.html",
...
test("deck presentation is complete and locally linked", () => {
  ...
  expect(gptQr).toMatch(new RegExp(`<desc>https://chatgpt\\.com/g/${gptId}-[^<]+</desc>`));
