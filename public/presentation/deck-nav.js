const slideDeck = [
  ["index.html", "Asisten Pribadi AI"],
  ["01-masalah.html", "Masalah pola"],
  ["02-solusi.html", "Satu backend, tiga klien"],
  ["03-demo.html", "Contoh: Custom GPT"],
  ["04-bukti.html", "Kontrak Action"],
  ["05-arsitektur.html", "Arsitektur"],
  ["06-kepercayaan.html", "Tulis yang bisa diaudit"],
  ["07-build.html", "Deploy sendiri"],
  ["08-penutup.html", "Penutup"],
];
const slides = slideDeck.map(([file]) => file);
// Export filenames stay `temanusaha-ai-*`: the binaries in exports/ were rendered before the
// rename and cannot be regenerated, so renaming these keys would 404 every download link.
...
// Canonical slug as of 2026-07-31. The old `-temanusaha-ai-warung-bu-sari` slug 301s here;
// verified with `curl -sI` that this URL answers 200 with no redirect hop.
const gptUrl = "https://chatgpt.com/g/g-6a5b0a5ef31c819181f8a68b5536d33e-asisten-waringa";

const current = Math.max(0, Math.min(slides.length - 1, Number(document.body.dataset.slide || 0)));
document.title = `${slideDeck[current][1]} — Asisten Pribadi AI`;
